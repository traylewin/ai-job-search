...
interface SourcesSidebarProps {
  ...
  calendarLastSyncDate?: string | null;
      ...
      <div className="flex-1 flex flex-col overflow-hidden px-2 py-3 gap-1">
          ...
          <div className="flex items-center shrink-0">
            ...
            {onRefreshJobStatuses && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (!jobStatusRefreshing) {
                    onRefreshJobStatuses(filteredJobs.map((j) => j.id));
                  }
                }}
                className={`mr-1 p-1 rounded transition ${jobStatusRefreshing ? "text-blue-400" : "text-gray-400 hover:text-blue-600 hover:bg-blue-50"}`}
                title="Refresh all job statuses with AI"
                disabled={jobStatusRefreshing}
              >
                <svg className={`w-3 h-3 ${jobStatusRefreshing ? "animate-spin" : ""}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <div className={`flex flex-col min-h-0 ${(openSections.emails || !!q) ? "flex-[1_1_auto] overflow-hidden" : "shrink-0"}`}>
          <div className="flex items-center shrink-0">
            ...
            {onRefreshEmail && emailConnected && (
              <button
                onClick={(e) => { e.stopPropagation(); onRefreshEmail(); }}
                disabled={emailSyncing}
                className="mr-1 p-1 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded transition disabled:opacity-50"
                title="Sync emails"
              >
                <svg className={`w-3 h-3 ${emailSyncing ? "animate-spin" : ""}`} fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
            ...
            <div className="overflow-y-auto min-h-0">
              {!emailConnected && onConnectEmail && (
                <button
                  ...
                  className="flex items-center gap-2 mx-2 mb-1 px-3 py-2 text-xs font-medium text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition w-[calc(100%-16px)]"
                >
              ...
              {emailConnected && onRefreshEmail && (() => {
                ...
                return (stale || emailSyncing) ? (
                  <button
                    ...
                    disabled={emailSyncing}
                    className="flex items-center gap-2 mx-2 mb-1 px-3 py-2 text-xs font-medium text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition w-[calc(100%-16px)] disabled:opacity-50"
                  >
                    <svg className={`w-3.5 h-3.5 shrink-0 ${emailSyncing ? "animate-spin" : ""}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182" />
                    </svg>
